...
async def run_task_episode(
    ...
    return {"score": score, "rewards": rewards}
...
async def main() -> None:
    ...
    try:
        ...
        for task_id in ["task_0", "task_1", "task_2"]:
            result = await run_task_episode(
                task_id=task_id,
                env=env,
                base_url=base_url,
            )
            # result is a dict {score, rewards}
            all_scores[task_id] = float(result.get("score", 0.0))
            task_reports[task_id] = {
                "score": all_scores[task_id],
    ...
    # Write outputs/report.json and simple HTML report
    try:
        out_dir = os.path.join(os.path.dirname(__file__), "..", "outputs")
        os.makedirs(out_dir, exist_ok=True)
        report_json = os.path.join(out_dir, "inference_report.json")
        with open(report_json, "w") as f:
            json.dump({"aggregate": agg, "scores": all_scores, "tasks": task_reports}, f, indent=2)

        # Simple HTML report
        report_html = os.path.join(out_dir, "report.html")
        html_content = [
            "<html><head><meta charset=\"utf-8\"><title>Inference Report</title></head><body>",
            f"<h1>Inference Report — Aggregate: {agg:.3f}</h1>",
            "<h2>Task Scores</h2>",
            "<ul>",
        ]
        for tid, data in task_reports.items():
            rewards_str = ','.join(f"{r:.2f}" for r in data['rewards'])
            html_content.append(f"<li><strong>{tid}</strong>: score={data['score']:.3f} — rewards={rewards_str}</li>")
        html_content.append("</ul>")
        html_content.append("<h2>Raw JSON</h2>")
        html_content.append(f"<pre>{json.dumps({'scores': all_scores, 'tasks': task_reports}, indent=2)}</pre>")
        html_content.append("</body></html>")
        with open(report_html, "w", encoding="utf-8") as f:
            f.write('\n'.join(html_content))
        # Also copy report to server static directory so demo UI can open it
        try:
            demo_static = os.path.join(os.path.dirname(__file__), "server", "static")
            if os.path.isdir(demo_static):
                target_html = os.path.join(demo_static, "report.html")
                with open(target_html, "w", encoding="utf-8") as tf:
                    tf.write('\n'.join(html_content))
                print(f"[DEBUG] Report written to {report_json} and {report_html} (copied to {target_html})", file=sys.stderr)
            else:
                print(f"[DEBUG] Report written to {report_json} and {report_html}", file=sys.stderr)
        except Exception:
            print(f"[DEBUG] Report written to {report_json} and {report_html}", file=sys.stderr)
    except Exception as e:
        print(f"[DEBUG] Failed to write report: {e}", file=sys.stderr)
